...
export class NewsPage {
  ...
  onFiltersSubmit() {
    ...
    const onlyConfirmed = this.filtersForm.get('onlyConfirmed')?.value || false;

    const tagsString = tags.length > 0 ? `"tags=${tags.join(',')}"` : '';
    const confirmedString = onlyConfirmed ? 'onlyConfirmed=true' : '';

    const queryParams = [tagsString, confirmedString]
      .filter((param) => param !== '')
      .join('&');
